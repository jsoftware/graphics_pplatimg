NB. pplatimg - image file I/O for specific platform
NB.
...
NB. =========================================================
NB. usage
NB.    r=: readimg_pplatimg_ 'D:/photo/u1_6002.jpg'
NB.    r writeimg_pplatimg_ 'D:/photo/u1_6003.jpg'
...
gdk_pixbuf_new_from_data   =: 'gdk_pixbuf_new_from_data    > x  *c x x  x x x  x  x x'gdpcd
...
assert gdk_pixbuf_save buf;y;type;(,0);0
gdk_pixbuf_unref buf
...
GdipCreateBitmapFromGdiDib=:'gdiplus GdipCreateBitmapFromGdiDib > i  x x *x       ' cdi
GdipCreateBitmapFromStream=:'gdiplus GdipCreateBitmapFromStream > i  x *x           ' cdi
GdipCreateBitmapFromFile =: 'gdiplus GdipCreateBitmapFromFile  > i   *w *x           ' cdi
GdipCreateBitmapFromScan0=: 'gdiplus GdipCreateBitmapFromScan0 > i   i i  i i  *i *x ' cdi
...
GdipGetImageEncodersSize =: 'gdiplus GdipGetImageEncodersSize  > i   *i *i           ' cdi
GdipGetImageEncoders     =: 'gdiplus GdipGetImageEncoders      > i    i  i  *c       ' cdi
...
NB. encoderParameters.Parameter[0].Guid = mEncoderQuality;
NB. encoderParameters.Parameter[0].NumberOfValues = 1;
NB. encoderParameters.Parameter[0].Type = EncoderParameterValueTypeLong;
NB. encoderParameters.Parameter[0].Value = &Quality;
...
NB. =========================================================
wstr=: (''"_)`([: ({.~ i.&(u:0)) 6: u: [:memr ,&(0,40))@.*

encoder=: 3 : 0
NUM=. ,_1 [ SZ=. ,_1
GdipGetImageEncodersSize NUM;SZ
ENCS=. (_76)]\SZ#'z'
GdipGetImageEncoders ({.NUM);({.SZ);ENCS
ENCS=. NUM{.ENCS
guids=. (#GUID){.("1) ENCS
infoaddr=. _2(3!:4)("1) ( (0,2*#GUID) ,: _,4*5 ) ];.0 ENCS
(1!:2&2)^:0 infos=. <@wstr("0) infoaddr
...
if. 3=#$y do.
  assert 3 e. $y
  y=. setalpha 256#.(|:~ (i.3) A.~ _1 + 2 ^ 2 - 3 i.~$) y
end.
y
)
